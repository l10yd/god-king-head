...
export const PLAYER = {
  ...
  BOOST_REGEN: 20,
  BOOST_MIN_USE: 12,
  /** ГИСТЕРЕЗИС буста: после опустошения шкалы ускорение НЕ отключается обратно,
   *  пока держишь SHIFT — тяга копится до этой планки и восстанавливается само.
   *  Лечит баг «замерло на 12% и не отъезжает». */
  BOOST_REARM: 35,
   ...
   *  Контакт НЕ снимает HP — он крадёт тягу: замедление + без SHIFT/дэша. */
  CURSE_DURATION: 2.4,
  CURSE_SLOW: 0.5,
} as const;
...
export const SCORE = {
  ...
  SPIRIT_PURGED: 75,
  /** очки за духа, СОЖЖЁННОГО лучом головы */
  SPIRIT_BURNED: 150,
  COMBO_MAX: 5,
  COMBO_WINDOW: 6.0,
} as const;

/** Красные духи: скрытое HP (игроку не показывается), сгорание в луче и красные волны */
export const SPIRITS = {
  /** HP духа; ~1.7 сек в луче до сгорания (быстро, но не мгновенно) */
  HP: 80,
  /** урон лучом по духу в секунду (пока дух в конусе луча) */
  BURN_PER_SEC: 48,
  /** полуугол конуса луча, в котором дух горит, рад (~21 м на сфере) */
  BURN_CONE: 0.09,
  /** мин. угловое расстояние «далеко от игрока» для мгновенного респауна (за спиной) */
  RESPAWN_MIN_ANGLE: 2.1,
  /** красная волна от сгоревшего духа: радиус/длительность/штраф замедления */
  WAVE_RADIUS: 46,
  WAVE_DURATION: 1.5,
  WAVE_CURSE: 1.6,
} as const;
...
export const DIFFICULTY = {  /** Первый красный дух не появляется раньше этого времени, сек */
  FIRST_RED_AT: 12,
